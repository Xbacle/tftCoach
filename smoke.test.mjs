...
console.log('\n=== T6: hỏi reroll 2 lần → dữ liệu GIỐNG HỆT (Gemini chỉ diễn đạt) ===')
...
console.log('context 2 lần giống hệt:', JSON.stringify(e1) === JSON.stringify(e2) ? 'OK' : 'SAI')
console.log('lọc levelling lvl:', e1.chunks.filter(c => c.type === 'comp').every(c => /lvl/i.test(c.levelling || '')) ? 'OK' : 'SAI')
const comp6 = e1.chunks.find(c => c.type === 'comp')
console.log('tên thân thiện:', comp6 ? comp6.name : '?', '| style:', comp6 ? comp6.style : '?', '| carry:', comp6 ? comp6.carryName : '?')
console.log('directAnswer để Gemini trình bày (thiết kế mới):', directAnswer(data, b1, e1) === null ? 'OK' : 'SAI')
